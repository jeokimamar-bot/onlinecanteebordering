...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if($insert_stmt->execute()) {
        $insert_stmt->close();
        header("Location: admin_users.php?success=Admin account created successfully!");
        exit();
    } else {
        $error_msg = $insert_stmt->error;
        $insert_stmt->close();
        header("Location: admin_users.php?error=Failed to create admin account. Error: " . htmlspecialchars($error_msg));
        exit();
    }
} else {
